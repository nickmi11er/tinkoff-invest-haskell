module Service.Internal.MarketDataStream
  ( MDStream
  , MDStreamMonad
  , STRequest (..)
  , STResponse (..)
...
import           Client
import           Client.Helpers          (ChanFlow (..))
...
import           Proto.Marketdata_Fields

data STRequest
  = PostRequest MarketDataRequest
  | Shutdown
data STResponse
...
data AppFault
  = BadFault SomeException
  | SomeOtherAppProblem
  deriving (Show)

data SubId
  = Candles
      { candlesInsts :: [(Text, SubscriptionInterval)]
      }
  -- (figi, interval)
  | OrderBook
      { orderInsts :: [(Text, Int32)]
      }
  -- (figi, depth)
  | Trades
      { figis :: [Text]
...
handleFault _ = pure () -- ignore interruption message
...
requestId request = case request ^. maybe'payload of
  ...
  Just (MarketDataRequest'GetMySubscriptions _)          -> Unknown -- What is the response?
  Nothing                                                -> Unknown
...
responseId response = case response ^.maybe'payload of
  ...
  Just (MarketDataResponse'Ping _)                        -> Unknown
  Nothing                                                 -> Unknown
